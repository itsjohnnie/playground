...
export function PulseGlow({ kind, duration = 1050 }: PulseGlowProps) {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      // then a long soft fade to 1. The peak intensity is held under 1
      // (multiplied by 0.7) so the hint never reaches its theoretical
      // ceiling — keeps the cue whispered rather than declarative.
